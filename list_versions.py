...
def read_go_version_from_dockerfile():
    # Read Go version from weave/build/Dockerfile
    dockerfile_path = path.join(path.dirname(path.dirname(path.realpath(__file__))), 'build', 'Dockerfile')
...
def parse_tag(tag, version_pattern, debug=False):
    ''' Parse Git tag output's line using the provided `version_pattern`, e.g.: 
    >>> parse_tag('915b77eb4efd68916427caf8c7f0b53218c5ea4a    refs/tags/v1.4.6', 'v(?P<version>\d+\.\d+\.\d+(?:\-beta\.\d)*)')
    '1.4.6'
    '''
...
def get_versions_from(git_repo_url, version_pattern):
    ...
    return list(filter(None, (parse_tag(line, version_pattern) for line in sanitize(out))))

def tree(versions, level=0):
    ...
    for _, versions_group in groupby(versions, lambda v: v.digits[level]):
        subtree = tree(list(versions_group), level+1)
        if subtree:
...
def is_iterable(obj):
    ...
    >>> is_iterable([1, 2, 3])
    True
    >>> is_iterable((x for x in [1, 2, 3]))
    True
    '''
    return hasattr(obj, '__iter__') and not isinstance(obj, str)

def leaf_versions(tree, rc):
    '''
    ...
    if is_iterable(tree):
        for subtree in tree:
            versions.extend(leaf_versions(subtree, rc))
        if not versions:
            if rc:
...
def filter_versions(versions, min_version=None, rc=False, latest=False):
    ...
    if latest:
        versions_tree = tree(versions)
        return leaf_versions(versions_tree, rc)
    else:
        return versions
...
def validate_input(argv):
    try:
        ...
        if dependency not in _DEPS.keys():
            raise ValueError('Please provide a valid dependency. Supported one dependency among {%s} but got: %s.' % (', '.join(_DEPS.keys()), dependency))
...
def main(argv):
    try:
        dependency, config=validate_input(argv)
        if dependency == 'go':
            try_set_min_go_version()
        versions = get_versions_from(_DEPS[dependency]['url'], _DEPS[dependency]['re'])
        versions = filter_versions(versions, _DEPS[dependency]['min'], **config)
